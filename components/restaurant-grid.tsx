...
export function RestaurantGrid({ restaurants, layout = "carousel", intent }: RestaurantGridProps) {
  const listKey = restaurants.map((restaurant) => restaurant.id).join("|");

  if (layout === "grid") {
    return (
      <Stagger key={listKey} play="mount" className="mt-8 grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
  ...
  return (
    <Stagger key={listKey} play="mount" className="mt-8 flex snap-x gap-6 overflow-x-auto pb-4 [scrollbar-width:thin]">
      {restaurants.map((restaurant) => (
